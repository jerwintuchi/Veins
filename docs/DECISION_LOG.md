...
## 2026-06-24 — Lobby join hardening + per-tab identity (sessionStorage)
**Context**: Reported as a string of lobby join failures. Three distinct bugs, fixed in order.
**1. "Stuck on Joining…" — `LOBBY_ERROR` listener never attached.** `LobbyScreen`'s error effect read `socketRef.current` and bailed when null with `[socketRef]` deps. Child effects run before parent effects, so on first mount `socketRef.current` was still null (the parent `useSocket` effect hadn't run); the listener was never attached and never re-ran (stable ref). Failed joins silently dropped → permanent "Joining…". Fix: add `connected` to the effect deps so it re-attaches once the socket exists. Also: Back now cancels a pending join, and an 8s timeout releases a hung request.
**2. "You are not in a room." spam on the lobby.** The client's global `mousemove`→`aim-player` and WASD→`move-player` listeners fired on every screen; the server replied `LOBBY_ERROR { NOT_IN_ROOM }` per event. Invisible until bug #1 was fixed, then every mouse-move on the lobby surfaced the error. Fix (both sides): server `move-player`/`aim-player` now **silently ignore** a missing room (per-frame input must not emit an error every frame — malformed-payload `INVALID_REQUEST` kept); client gates those emits behind an `inGameRef` so nothing is sent from the lobby. `tickLoop.test.ts` updated (silent no-op, not `LOBBY_ERROR`).
**3. "You are already in that room." — same-browser self-join.** The stable playerId lived in `localStorage`, shared across all tabs, so a second tab joining the host's room collided (`ALREADY_IN_ROOM`) — you were joining yourself, and local multi-client testing was impossible. Fix: store it in **`sessionStorage`** (per-tab, but survives reload). Two tabs are now distinct players; a refresh still keeps the id so reconnection (R1/R6) holds; identity lifetime now matches `veins.roomCode` (already sessionStorage). Reconnection spec R1 + design updated.
**Test-locally note**: open the second player in a **new tab/window** (fresh sessionStorage), not a duplicated tab (browsers copy sessionStorage on duplicate).
**Result**: 410 server + 181 client tests passing. Zero new typecheck errors (baselines unchanged: server 41, client 13).

---

## 2026-06-24 — Leave Room transition + lobby resilience
**Leave Room did nothing (host stuck).** `WaitingRoom` only emitted `leave-room`; the server removes the player (and deletes a now-empty room) but sends the leaver no ack, so the client never left the screen. Fix: `WaitingRoom` takes an `onLeave` callback — it emits `leave-room` then immediately returns to the lobby (App clears `roomSummary`, forgets the room code). Server `leave-room` now also `socket.leave(code)` so the leaver stops getting that room's broadcasts (otherwise a later `ROOM_UPDATE` would re-yank them into the waiting screen via App's `onRoomUpdate`).
**Stale room code recovery.** Lobby rooms are torn down when the host's socket drops, and rooms are ephemeral (I7) — so a server restart (incl. dev auto-restart) or a host reconnect leaves a dead code. A failed auto-`rejoin` now drops the stored code and returns to the lobby (App tracks `pendingRejoinRef` so this is distinguished from a manual join failure, which LobbyScreen still surfaces). The genuine "No room with that code" on a wiped room is correct — recreate the room.

---

## 2026-06-24 — Enemies not rendering + movement fluidity
**Enemies never appeared (floor 1).** The server emitted floor-1 `ENEMY_SPAWNED` **synchronously right after `RUN_STARTED`**, but the Phaser `GameScene` binds its socket listeners only after React swaps to the game screen and Phaser boots — so those events fired before any listener existed and were lost (the exact "fires before Phaser is ready" issue already worked around for dungeon/players via the game registry; enemies were omitted). Fix: floor-1 enemies now ride the `RUN_STARTED` payload (`enemies: {enemyId,typeId,x,y,hp}[]`, added to `RunStartedEvent`), App stashes them in `game.registry['initialEnemies']`, and `GameScene.create()` spawns them — same mechanism as dungeon/playerPositions. The redundant `ENEMY_SPAWNED` loop is kept (idempotent client-side) for any already-live scene.
**Closes the resync follow-up.** `App.onStateResync` now also passes `ev.enemies` into `runData`, so reconnecting into an in-progress run rehydrates enemy sprites via the same registry path. (Projectile sprite rehydration on resync remains a minor open item — projectiles are sub-second-lived.)
**Movement fluidity.** `COMBAT_TICK_MS` 100 → 50 (10Hz → 20Hz). All combat math is dt-based, so gameplay speed is unchanged; position updates (`PLAYER_MOVED`/`ENEMY_MOVED`) now arrive twice as often, roughly halving input-to-render latency and smoothing the client's existing interpolation. Client interpolation left as-is. Further smoothness (local-player client-side prediction) remains a deferred option per the netcode "add prediction later if needed" decision.
**Result**: 60 shared + 410 server + 182 client tests passing. Zero new typecheck errors (baselines unchanged: shared 4, server 41, client 13).

---

## 2026-06-24 — Combat audit: aim-ring bug fixed; client-prediction spec written; active spec switched
**Audit.** Read-only sweep of the combat path (`combat/{tick,roomCombat,movement,weapon,spawn,separation,autoAim}.ts`, `index.ts` `runCombatTick`, `GameScene.ts`) → categorized punch list. Most of the suite is solid; findings split into bugs / feel-polish / design-balance.
**Bug fixed (B1+B2) — auto-aim ring.** The ring was positioned only on `PLAYER_AIM_CHANGED`, never in `GameScene.update`, so it froze in place as the target enemy lerped (B1); and `killEnemy` hid the ring on *any* enemy death, so killing an unrelated enemy dropped a live lock until the target changed (B2). Fix: track `localAimTargetId`, re-pin the ring to that enemy's live position every frame in `update`, and hide only when the target itself is gone; `killEnemy` clears the lock only if the dead enemy *was* the target. Two regression tests added to `GameScene.test.ts` (follows moving target; survives unrelated deaths, hides on target death). 184 client tests pass; tsc unchanged (13).
**Deferred bug (B3, minor).** An enemy reduced to 0 HP by a projectile *this tick* still acts once before `tickEnemies` marks it dead (mark happens after the act loop). Low impact; reordering risks other tests — tracked, not fixed.
**Spec written (via spec-writer) — `specs/client-prediction/`.** Local-player client-side movement prediction with smooth/firm server reconciliation. Client-only, **no new socket events, no new authority** (reuses `move-player` out / `PLAYER_MOVED` in — honors I1/I2/I6); server untouched. R1–R6 + P1–P4 (parity via shared `PLAYER_SPEED`, convergence, authority, scope isolation), T1–T4 (input plumbing → predict → reconcile → phase-gate). v1 deliberately has **no client-side collision** (I4 keeps collision server-side; brief wall-overshoot is absorbed by the `RECONCILE_SNAP_DISTANCE` firm-correction branch); client collision mirror tracked as future. **Active Spec in CLAUDE.md switched → client-prediction.** Not yet implemented — spec only.

---

## 2026-06-24 — Desktop control scheme: manual mouse-aim + hold-to-fire (revises Auto-Aim decision)
**Decision.** On **desktop only**, replace auto-aim+auto-fire with **cursor aim (sticky, no auto-revert) + hold-left-mouse-button to fire**. **Mobile is unchanged** (auto-aim + auto-fire). This revises the 2026-06-14 "Auto-Aim with Manual Override" decision for desktop, per a playtest request — auto-fire on every tick + aim reverting to nearest-enemy after 500ms idle made desktop aiming feel like it "wouldn't aim at the mouse" and "fired on mouse-move."
**Server (trust boundary preserved, I2).** New `Room.playerFiring?: Map<PlayerId, boolean>` (init `true` per player in `startRun`; created empty in `createRoom`). New `set-firing { firing: boolean }` socket handler — validated, silently ignored when not in a room (per-frame-ish input, like `move-player`/`aim-player`). `tryAutoFire` returns null when `playerFiring.get(pid) === false`; **absent/true = auto-fire**, so existing weapon tests and mobile are unaffected (optional-chained read). Cooldown still drains while opted-out so the first held shot fires immediately, and the server still rate-limits — no client-authored projectiles.
**Client (desktop, gated on `matchMedia('(pointer: fine)')`).** Removed the 500ms aim auto-revert (cursor aim is now sticky); aim is recomputed from the cursor on mouse-move AND each frame while strafing (player→cursor vector changes as you move). On entering the game the desktop client emits `set-firing false` (opt out of auto-fire), then `mousedown`(button 0)→`true`, `mouseup`/`blur`→`false`. Mobile never runs this path.
**Tests.** Server: `weapon.test.ts` (opted-out → no fire; holding → fires; absent → auto-fire) + `tickLoop.test.ts` (`set-firing` sets flag, malformed ignored, no-room ignored). Client: `App.test.tsx` (game-entry opt-out emits `set-firing false`; LMB down/up → true/false; right-click ignored; aim does NOT auto-revert after idle). GLOSSARY "Auto-Aim" updated.
**Result**: 416 server + 187 client tests passing. Zero new typecheck errors (baselines unchanged: server 41, client 13).
**Future:** mobile fire control (a fire button / full manual) if desired; client-side aim assist toggle. Melee weapons can hook the same `playerFiring` intent.

---

## 2026-06-24 — Two render bugs: aim origin not seeded; resync didn't re-grey downed players
**Aim offset (firing not at cursor).** `GameScene.addOrUpdatePlayer` never seeded `sceneStore.localPlayerPos` on the local spawn, so until the first `PLAYER_MOVED` the mouse-aim fell back to a screen-centre estimate. That estimate is wrong whenever the camera is off-centre — e.g. near a dungeon edge (camera bounds-clamp) or a just-spawned/just-switched-to player who hasn't moved — producing an angular aim offset. Fix: seed `sceneStore.localPlayerPos = { x, y }` on local spawn so the accurate `getWorldPoint(cursor) - playerPos` path is used immediately. Regression tests in `GameScene.test.ts` (local spawn seeds it; remote spawn does not).
**Resync rendered downed players as alive (blue).** `STATE_RESYNC` rebuilds the scene from the snapshot via the registry, but `PLAYER_DOWNED` events are not replayed, so downed players were re-spawned in the normal (blue) colour even though the server still (correctly) blocked their movement. Fix: `App.onStateResync` derives downed player ids from `ev.playerStates` and passes them via `game.registry['initialDownedPlayers']`; `GameScene.create()` calls `downPlayer` for each. Tests: `App.reconnect.test.tsx` (resync passes `initialDownedPlayers`), and `downPlayer`'s grey already covered.
**Still open (noted to user):** (1) the revive PANEL (BoardPanel, React) is not restored on resync — it only reacts to live `PLAYER_DOWNED`; rehydrating it from the snapshot is a separate follow-up. (2) Enemy attacks have **no rendered visual** — shamblers are melee (instant) and spitters are instant hitscan with no projectile entity; an enemy-attack telegraph/projectile is a feature (audit item F4), needs a new server event.
**Result**: 190 client tests passing; tsc unchanged (13).

---

## 2026-06-24 — Circulatory Board viewable during combat (was loot-only / invisible)
**Problem.** The `BoardPanel` returned `null` whenever `phase !== 'loot'` (unless reviving), so during combat the board — and any sense of where relic slots are — was completely invisible. A player who never cleared a floor never saw it and assumed it was non-interactive / had no visuals.
**Change.** The board now always renders. Outside loot it defaults to a **collapsed "▲ Relic Board" button** (`data-testid="board-minimized-btn"`) the player can expand to **view their board read-only** (slots, placed relics, synergies) with a `board-viewonly-hint` ("place relics after clearing the floor"). It **auto-expands on entering loot** (and on a revive) and auto-collapses otherwise, via an effect keyed on `[phase, revive.active]`; the player can still toggle within a phase. Placement remains loot-only (the relic tray is still `phase === 'loot'`-gated, and `selected` can only be set from the tray), so this is a pure visibility win with no change to when relics can be placed. The empty-slot hexes (gray outlines; white outline when a relic is selected) were already there — they were just hidden during combat.
**Tests.** `BoardPanel.test.tsx`: collapsed-in-combat + expand-to-view-only (no tray), auto-expanded in loot (no view-only hint), and combat→loot rerender auto-expands.
**Result**: 192 client tests passing; tsc unchanged (13).

---

## 2026-06-28 — Revive allowed in loot (revises R11 combat-only); descend reposition; silent-error fix
**Revive phase (revises Enemy Combat R11 "revive combat-only").** Linked Fates is now allowed in **both `combat` and `loot`** (disallowed only during the brief `transition`). Combat-only created a dead-end: a teammate downed as the fight ended was stranded through the whole loot phase and descended incapacitated — and the rejection was **silent** because it emitted `LOBBY_ERROR` (which `BoardPanel` doesn't render) instead of `LINKED_FATES_ERROR`. The test arena (fast clears) made this the common case. Fix: relax the phase guard and route the rejection through `LINKED_FATES_ERROR` (new `WRONG_PHASE` code on `LinkedFatesErrorEvent`) so the revive panel surfaces it. Client also hides the placement tray while a revive is in progress, so loot+revive isn't a cluttered dual-UI (matches how combat-revive already looked). `tickLoop.test.ts` revive-in-loot test flipped from "rejects" to "succeeds"; added a transition-phase rejection test.
**Descend repositioning (regression fix).** Descend never moved players — they kept their previous-floor coordinates onto the new floor. In the arena they landed on top of the fresh enemy spawns: enemy sprites (`depth 4`) render behind player sprites (`depth 5`), so the enemies looked absent while dealing immediate damage ("DPS, no enemies"). In the real game it could drop players inside a wall. Fix: `descendRoom` repositions every player to the new floor's entry-room centre (as `startRun` does), HP/downed carried over; positions ride a new optional `FloorAdvancedEvent.playerPositions` and the client `snapPlayer`s sprites instantly on `FLOOR_ADVANCED` (snap, not lerp — new floor).
**Per-player loot + one placement per phase (gameplay rules).** `room.lootPool: RelicId[]` → `room.lootPools: Record<PlayerId, RelicId[]>` (each player an independent pool, keyed by id; clients read their own by `localPlayerId`). A player may place **one relic per loot phase** (`room.placedThisLootPhase`, reset on each loot entry); placing clears the rest of that player's pool. `DOWNED` and `ALREADY_PLACED` error codes added; downed players can neither place nor revive.
**Test arena (test-only, `VEINS_TEST_ARENA=1`).** Single open room + one enemy per type for fast loot iteration; off by default, gated so production uses the real `generateDungeon`/`spawnEnemies`. `src/server/src/dungeon/testArena.ts`.
**Result**: 60 shared + 436 server + 200 client tests passing.

---

## 2026-06-28 — Stack pivot: Godot (GDScript) client replaces React + Phaser; Node server stays authoritative

**Decision.** Migrate the client renderer from React + Phaser 3 to a **Godot 4 project (GDScript)**, exported to **HTML5/WASM** so the browser-first "open a URL and play" promise (pitch.md) is preserved. The **authoritative Node + Socket.io server (`src/server`, ~3,000 LOC) and `src/shared` are unchanged**: the trust boundary (I1/I2) and all server tests stand. Pixel art is authored in **Aseprite** and imported via sprite sheets / the Godot Aseprite import plugin. Motivation: richer visual authoring and a Godot-MCP-driven dev loop. Supersedes the Phaser renderer choice recorded in technical/stack-and-deployment.md.

**Why this is structurally low-risk.** `registerHandlers(io, manager)` already depends only on a thin abstract transport surface (`SocketIOServerLike` / `ServerSocket` in index.ts: connection, `to(room).emit`, per-socket `emit`/`on`/`join`, `handshake.auth`). The game core never imports Socket.io directly, so the renderer and transport can be swapped without touching game logic.

**Transport.** Godot speaks raw WebSocket (`WebSocketPeer`), not Socket.io's protocol. Rather than bridge socket.io-client via JavaScriptBridge (web-export-only, fragile), add a **raw-WebSocket adapter that implements the existing `SocketIOServerLike`/`ServerSocket` interfaces** (rooms as `Map<code, Set<conn>>`, messages as a JSON envelope `{ event, payload }`, stable-id reconnection via a `playerId` auth frame or query param). It registers the same `registerHandlers` against the same `RoomManager`, so it runs **alongside** the Socket.io server during migration (old React client and new Godot client both live), then Socket.io is retired. Zero changes to the game core.

**src/shared single-source-of-truth.** Godot cannot import the TS constants/types in `@veins/shared` (the I4 gate that keeps client render and server sim from diverging). Mitigation: a **codegen step** emits a GDScript constants file (`PLAYER_SPEED`, tick intervals, `CORRIDOR_HALF_WIDTH`, event-name strings) from `src/shared`, so both runtimes still derive from one source. Manual duplication is rejected: it is exactly the drift I4 exists to prevent.

**Active spec impact.** `specs/client-prediction` is renderer-coupled (GameScene.ts / SceneStore.ts / Phaser) and is **mooted as written**. Its requirements (R1-R6, P1-P4: immediate local response, smooth/firm reconciliation, scope isolation, shared-`PLAYER_SPEED` parity) survive conceptually and will be re-expressed against the Godot client. A `specs/godot-client/` spec (requirements, design, tasks) will be authored before implementation, per the spec workflow; Active Spec in CLAUDE.md rotates when it lands.

**Accepted risks (web export).** Godot HTML5 bundles are multi-MB and historically fragile on mobile Safari (SharedArrayBuffer / COOP-COEP headers, audio). These hit the actual target (mobile web) and must be validated early with a smoke export, not at the end. The $0 hosting story (static Godot export on Vercel, Fly.io WS server) is unchanged in principle.

**Result:** Decision logged; no code changed yet. Next: author `specs/godot-client/`, then the raw-WS transport adapter (server-side, additive) as the first implementation step.
